...
// Rolling throughput counter, mirroring the reference's Stats: a round is `round_size`
// messages timed from the first to the (round_size+1)-th message.
class Stats {
public:
    explicit Stats(std::size_t round_size) : round_size_(round_size), round_start_(Clock::now()) {}
    ...
    auto increment() -> void {
        if (round_count_ == 0) {
            ...
            ++round_count_;
        } else if (round_count_ < round_size_) {
            ++round_count_;
        } else {
            print_round();
            ++finished_rounds_;
    ...
    [[nodiscard]] auto finished_rounds() const -> std::size_t { return finished_rounds_; }
...
auto main(int argc, char** argv) -> int {
    ...
    auto sub = session->declare_subscriber(
        "test/thr",
        [&stats, samples](const zenoh::Sample&) {
            stats.increment();
            if (stats.finished_rounds() >= samples) {
                stats.print_summary();
                std::exit(0);
            }
        },
